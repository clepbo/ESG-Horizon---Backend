...
import { AmmoniaUreaComputationDto, BasicComputationDto, BusinessTravelDto, CementComputationDto, EmployeeCommutingDto, EndOfLifeTreatmentSoldProductsDto, FlaringComputationDto, FranchiseEnergyInputDto, FuelComputationsDto, FuelEnergyRelatedActivitiesDto, GoodsAndServicesDto, NationalGridComputationDto, OilGasVentingDto, RefigirantComputationDto, UpstreamTransportationDistributionDto, UseSoldProductsDto, WasteGeneratedOperationsDto } from './dto/create-computation.dto';
...
async LPGComputations(dto: {energy_types: Array <BasicComputationDto>}){
  const value = dto.energy_types.reduce((sum, fuel) => {
...
async oilGasVentingComputations( dto: OilGasVentingDto ){
  const value = dto.gas_volume_released * (dto.methane_density || 0.656) * (dto.global_warming_potential || 28) /1000;
  return {
    value,
    unit: 'tCO2​e',
  };
...
async fuelEnergyRelatedActivitiesComputations(dto: FuelEnergyRelatedActivitiesDto){
  const value = dto.total_energy_spent * dto.upstream_lifecycle_emission_factor/1000;
  return {
    value,
    unit: 'tCO2​e',
  };
...
async upstreamTransportationDistributionComputations(dto: UpstreamTransportationDistributionDto){
  const value = dto.distance * dto.mass_of_goods * dto.emission_factor /1000;
  return {
    value,
    unit: 'tCO2​e',
  };
...
async employeeCommutingComputations(dto: EmployeeCommutingDto){
  const value = dto.number_of_employee * dto.average_daily_commuting_distance * dto.emission_factor_of_commuting_mode /1000;
  return {
    value,
    unit: 'tCO2​e',
  };
...
async downstreamTransportationDistributionComputations(dto: BasicComputationDto){
  const value = dto.value * dto.emission_factor /1000;
  return {
    value,
    unit: 'tCO2​e',
...
async useSoldProductsComputations(dto: UseSoldProductsDto){
  const value = dto.product_lifetime_energy_consumption * dto.number_of_units_sold * dto.regional_grid_emission_factor * dto.estimated_average_product_lifetime /1000;
  return {
    value,
    unit: 'tCO2​e',
  };
...
async endOfLifeTreatmentSoldProductsComputations(dto: EndOfLifeTreatmentSoldProductsDto){
  const value = dto.total_mass_of_products_sold * dto.emission_factor_for_end_of_life_treatment_method /1000;
  return {
    value,
    unit: 'tCO2​e',
  };
...
async franchisesComputations(energies: FranchiseEnergyInputDto[]) {
 const breakdown: { type: string; energy_consumed: number; emission_factor: number; value: number; unit: string; }[] = energies.map(e => ({
    type: e.type,
