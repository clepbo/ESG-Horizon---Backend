...
export class ComputationController {
  ...
  @ApiOperation({summary: "Greenhouse gases released during industrial or chemical processes, not from fuel combustion."})
  @ApiBody({type: ProcessEmissionDto})
  async processEmission(
    @Body() dto: ProcessEmissionDto
  ){
  ...
  @ApiOperation({
    summary:"Unplanned releases of gases from equipment, pipelines, storage tanks, or processes, including methane leaks, gas venting, flaring inefficiencies, and refrigerant losses."
  })
  @ApiBody({
    type: FugitiveEmissionCalculationDto
  })
  async fugutuveEmission(
    @Body() dto: FugitiveEmissionCalculationDto
  ){
  ...
  async locationBasedEmission(
    @Body() dto: LocationBasedEmissionDto
  ){
  ...
  async marketBasedEmission(
    @Body() dto: MarketBasedEmissionDto
  ){
